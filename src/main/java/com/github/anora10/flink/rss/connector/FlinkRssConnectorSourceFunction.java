...
public class FlinkRssConnectorSourceFunction extends RichParallelSourceFunction<RowData>
  ...
  public FlinkRssConnectorSourceFunction (DeserializationSchema<RowData> deserializationSchema, String[] uris, Integer refreshInterval) {
      ...
      for (String uri : urisToProcess) {
        try {
          ...
          if (response.getStatus() != 200) {
            throw new RuntimeException("Failed at uri: "
                    + uri
                    + " HTTP error code : "
                    + response.getStatus());
          }
          ...
          ByteArrayInputStream responseAsBytes = new ByteArrayInputStream(
          ...
          for (int i = 0; i < itemNodeList.getLength(); i++) {
            ...
            if (itemNode.getNodeType() == Node.ELEMENT_NODE) {
              ...
              RowData row = deserializer.deserialize(RssConnectorUtils.transformXmlToString(itemElement).getBytes());
              // Bloom filtering to remove duplicates with a high probability
              if (context != null && !bloomFilter.mightContain(row.toString().getBytes())) {
                bloomFilter.put(row.toString().getBytes());
                context.collect(row);
              }
            }
          }
